...
pki --self --in ~/pki/private/ca.key.pem --type sm2 --digest sm3 --dn "C=China, O=Tsinghua, CN=VPN" --ca --lifetime 3650 --outform pem > ~/pki/x509ca/ca.cert.pem
...
pki --pub --type sm2 --in ~/pki/private/server.key.pem --outform pem > ~/pki/pubkey/server.pub.key.pem

#For VPN server certificate key
pki --req --in ~/pki/private/server.key.pem --type sm2 --dn "C=China, O=Tsinghua, CN=VPN" --digest sm3 --outform pem | pki --issue --type pkcs10 --digest sm3 --lifetime 1200 --cacert ~/pki/x509ca/ca.cert.pem --cakey ~/pki/private/ca.key.pem --flag serverAuth --flag ikeIntermediate --outform pem  --san="120.79.52.57" > ~/pki/x509/server.cert.pem
...
pki --req --in ~/pki/private/client.key.pem --type sm2 --dn "C=China, O=Tsinghua, CN=VPN" --digest sm3 --outform pem | pki --issue --type pkcs10 --digest sm3 --lifetime 1200 --cacert ~/pki/x509ca/ca.cert.pem --cakey ~/pki/private/ca.key.pem --outform pem  --san="120.79.19.223" > ~/pki/x509/client.cert.pem
