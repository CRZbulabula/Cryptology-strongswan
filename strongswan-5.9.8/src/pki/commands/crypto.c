...
int gmsdf_dh_test(chunk_t data)
{
	...
	dh_i = lib->crypto->create_ke(lib->crypto, group);
	if(!dh_i)
	{
	...
	dh_r = lib->crypto->create_ke(lib->crypto, group);
	if(!dh_r)
	{
	...
	ret = dh_i->get_public_key(dh_i, &pub_i);
	ret = dh_r->get_public_key(dh_r, &pub_r);

	ret = dh_i->set_public_key(dh_i, pub_r);
	ret = dh_r->set_public_key(dh_r, pub_i);
